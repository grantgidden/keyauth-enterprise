...
if (isset($_POST['draw'])) {
        ...
        $columnIndex = misc\etc\sanitize($_POST['order'][0]['column']) -1; // Column index
	...
	$sel = misc\mysql\query("select count(1) as allcount from `whitelist` where app = ?", [$_SESSION['app']]);
	...
	if (!is_null($searchValue)) { // don't double query if no search value was provided
		...
		$sel = misc\mysql\query("select count(1) as allcount from `whitelist` WHERE 1  and (`ip` like ?) and app = ?", ["%" . $searchValue . "%", $_SESSION['app']]);
		$records = mysqli_fetch_assoc($sel->result);
		$totalRecordwithFilter = $records['allcount'];
	}

        // whitelist certain column names and sort orders to prevent SQL injection
        if (!in_array($columnName, array("ip"))) {
        ...
        if (!is_null($searchValue)) {
                $query = misc\mysql\query("select * from `whitelist` WHERE (`ip` like ?) and app = ? order by `" . $columnName . "` " . $columnSortOrder . " limit " . $row . "," . $rowperpage, ["%" . $searchValue . "%", "%" . $searchValue . "%", "%" . $searchValue . "%", $_SESSION['app']]);
        }
        else {
                $query = misc\mysql\query("select * from `whitelist` WHERE app = ? order by `" . $columnName . "` " . $columnSortOrder . " limit " . $row . "," . $rowperpage, [$_SESSION['app']]);
        }
        ...
        while ($row = mysqli_fetch_assoc($query->result)) {
            $data[] = array(
                "ip" => '<span class="blur-sm hover:blur-none">' . ($row["ip"]) . '</span>',
                "actions" =>
                    '<form method="POST">
                        <td>
                            <div x-data="{ open: false }" class="z-0">
                                ...
                                <ul x-show="open" x-on:click.away="open = false" class="bg-[#09090d] text-white rounded shadow-lg absolute py-2 mt-1" style="min-width:15rem">
                                    <li>
                                        <button name="deletewhite" class="block hover:opacity-60 whitespace-no-wrap py-2 px-4 hover:text-red-700" value="' . ($row["ip"]) . '">
                                            Delete Whitelist
                                        </button>
                                    </li>
                                </ul>
