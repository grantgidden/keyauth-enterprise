<nav class="flex mb-5" aria-label="Breadcrumb">
    <ol class="inline-flex items-center space-x-1 md:space-x-2">
        ...
        <?php
           if (($expires - time() <= 7 * 24 * 60 * 60) && !in_array($role, ['Reseller', 'Manager'])) {
            ?>
                <i class="lni lni-angle-double-right mr-2"></i>
                <label class="inline-flex items-center text-sm font-medium text-red-600">
                    <?= ($role == "tester" ? "You don't have a subscription!" : "Your subscription expires soon!") ?>
                    <a href="https://<?= $_SERVER['HTTP_HOST'] ?? $_SERVER['SERVER_NAME'] ?>/app/?page=upgrade" class="text-blue-600 hover:underline">
                        &nbsp;Upgrade Now.
                    </a>
                </label>
            <?php } 
