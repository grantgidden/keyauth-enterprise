...
if (isset($_POST['addsub'])) {
    if (!is_numeric($_POST['level'])) {
        ...
        match($resp){
            'already_exists' => dashboard\primary\error("This subscription already exists!"),
...
if (isset($_POST['pausesub'])){
    $resp = misc\sub\pause($_POST['pausesub']);
    match($resp){
        'failure' => dashboard\primary\error("Failed to pause subscription!"),
        'success' => dashboard\primary\success("Successfully paused subscription!"),
        default => dashboard\primary\error("Unhandled Error! Contact us if you need help")
    };
}

if (isset($_POST['unpausesub'])){
    $resp = misc\sub\unpause($_POST['unpausesub']);
    match($resp){
        'failure' => dashboard\primary\error("Failed to unpause subscription!"),
        'success' => dashboard\primary\success("Successfully unpaused subscription!"),
        default => dashboard\primary\error("Unhandled Error! Contact us if you need help")
    };
}
...
if (isset($_POST["selected"]) && isset($_POST["action"])) {
    ...
    if ($_POST["action"] == "delete") {
        foreach ($selected as $sub) {
            $resp = misc\sub\deleteSingular($sub);
            match($resp){
                'failure' => dashboard\primary\error("Failed to delete subscription"),
                'success' => dashboard\primary\success("Successfully deleted subscription"),
                default => dashboard\primary\error("Unhandled Error! Contact us if you need help")
    ...
    else if ($_POST["action"] == "pauseMulti") {
        foreach ($selected as $value) {
            $resp = misc\sub\pause($value);
            match($resp){
                'failure' => dashboard\primary\error("Failed to pause subscriptions!"),
                'success' => dashboard\primary\success("Successfully paused subscriptions!"),
                default => dashboard\primary\error("Unhandled Error! Contact us if you need help")
            };
        }
    }
    else if ($_POST["action"] == "unpauseMulti") {
        foreach ($selected as $value) {
            $resp = misc\sub\unpause($value);
            match($resp){
                'failure' => dashboard\primary\error("Failed to unpause subscriptions!"),
                'success' => dashboard\primary\success("Successfully unpaused subscriptions!"),
                default => dashboard\primary\error("Unhandled Error! Contact us if you need help")
            };
        }
    }
    ...
    <div class="mb-1 w-full bg-[#0f0f17] rounded-xl">
        <div class="mb-4 p-4">
            ...
            <p class="text-xs text-gray-500">Subscriptions act as levels/tiers. <a
                    href="https://keyauthdocs.apidog.io/sellerapi/subscriptions/create-new-subscription" target="_blank"
            ...
            <div class="p-4 flex flex-col">
                <div class="overflow-x-auto">
                    ...
                    <button id="dropdownselection" data-dropdown-toggle="subscriptiondropdown" 
                    ...
                    <div id="subscriptiondropdown" class="z-10 hidden bg-[#09090d] divide-y divide-gray-100 rounded-lg shadow w-44">
                        <ul class="py-2 text-sm text-white">
                            ...
                            <li>
                                <form method="post">
                                    <input type="hidden" name="action" value="pauseMulti">
                                    <input type="hidden" id="selected" name="selected">
                            ...
                            <li>
                                <form method="post">
                                    <input type="hidden" name="action" value="unpauseMulti">
                                    <input type="hidden" id="selected" name="selected">
                                ...
                                if ($_SESSION['app']) {
                                    ...
                                    if ($query->num_rows > 0){
                                        while ($row = mysqli_fetch_array($query->result)){
                                            echo "<tr>";
                                            echo '<td> <input type="checkbox" style="outline:none;"class="w-4 h-4 text-blue-600 bg-[#0f0f17] border-gray-300 rounded focus:ring-blue-500 checkbox" value="' . $row["name"] .'">' . '</td>';
                                            echo "<td>" . $row["name"] . "</td>";
                                            ...
                                            <ul x-show="open" x-on:click.away="open = false" class="bg-[#09090d] text-white rounded shadow-lg absolute py-2 mt-1" style="min-width:15rem">
                                                    ...
                                                    <li>
                                                            <button name="pausesub" class="block hover:opacity-60 whitespace-no-wrap py-2 px-4 hover:text-red-700"
                                                            value="' . $row["name"] . '">
                                                            Pause Subscription
                                                            </button>
                                                    </li>
                                                    <li>
                                                            <button name="unpausesub" class="block hover:opacity-60 whitespace-no-wrap py-2 px-4 hover:text-red-700"
                                                            value="' . $row["name"] . '">
