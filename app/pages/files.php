...
if (isset($_POST['addfile'])) {
    ...
    match($resp){
        ...
        'invalid_extension' => dashboard\primary\error("No HTML files. You must get the DIRECT URL which downloads INSTANTLY. No download page, direct file."),
        'bad_response' => dashboard\primary\error("File either not found or URL not allowed by the file host!"),
        'discord_cdn' => dashboard\primary\error("Discord's developers made files expire after 24 hours, use catbox.moe or pages.dev instead!"),
        'github_no_raw' => dashboard\primary\error("Click the \"Raw\" button on GitHub file. Only use raw.githubusercontent.com URLs"),
...
if (isset($_POST['editfile'])) {
    ...
    $result = misc\mysql\query("SELECT `url` FROM `files` WHERE `id` = ? AND `app` = ?", [$file, $_SESSION['app']]);
    $fileUrl = mysqli_fetch_array($result->result);
                ...
                <div class="px-6 py-6 lg:px-8">
                    ...
                    <form class="space-y-6" method="POST">
                        ...
                        <input type="text" id="url" name="url"
                            ...
                            placeholder=" " autocomplete="on" value="' . $fileUrl['url']. '"required>
                        <label for="url"
...
if (isset($_POST['savefile'])) {
    ...
    if (str_contains($url, "discord")) {
        dashboard\primary\error("Discord's developers made files expire after 24 hours, use catbox.moe or pages.dev instead!");
        echo "<meta http-equiv='Refresh' Content='2;'>";
        return;
    }

    if (str_contains($url, "github.com")) {
        dashboard\primary\error("Click the \"Raw\" button on GitHub file. Only use raw.githubusercontent.com URLs");
        echo "<meta http-equiv='Refresh' Content='2;'>";
        return;
    }
    ...
    if ($httpStatusCode == 403 || $httpStatusCode == 404) {
        dashboard\primary\error("File either not found or URL not allowed by the file host!");
        echo "<meta http-equiv='Refresh' Content='2;'>";
        return;
    }
    ...
    if(str_contains($contType, "html")) {
        dashboard\primary\error("No HTML files. You must get the DIRECT URL which downloads INSTANTLY. No download page, direct file.");
        echo "<meta http-equiv='Refresh' Content='2;'>";
        return;
    }
...
if (isset($_POST["selected"]) && isset($_POST["action"])) {
    ...
    if ($_POST["action"] == "delete") {
        foreach ($selected as $value) {
            $resp = misc\upload\deleteSingular($value);
            match($resp){
                'failure' => dashboard\primary\error("Failed to delete file!"),
                'success' => dashboard\primary\success("Successfully deleted file!"),
                default => dashboard\primary\error("Unhandled Error! Contact us if you need help")
            };
        }
    }
    else if ($_POST["action"] == "authEnable") {
        foreach ($selected as $value) {
            $query = misc\mysql\query("UPDATE `files` SET `authed` = 1 WHERE `app` = ? AND `id` = ?", [$_SESSION['app'], $value]);

            if ($query->affected_rows != 0) {
                misc\cache\purge('KeyAuthFile:' . ($secret ?? $_SESSION['app']) . ':' . $fileid);
                dashboard\primary\success("Successfully updated files!");
            } else {
                dashboard\primary\error("Failed to update files");
            }
        }
    }
    else if ($_POST["action"] == "authDisable") {
        foreach ($selected as $value) {
            $query = misc\mysql\query("UPDATE `files` SET `authed` = 0 WHERE `app` = ? AND `id` = ?", [$_SESSION['app'], $value]);
    ...
    <div class="mb-1 w-full bg-[#0f0f17] rounded-xl">
        <div class="mb-4 p-4">
            ...
            <p class="text-xs text-gray-500">Let your users download files you upload here. <a
                    href="https://keyauthdocs.apidog.io/api/features/download-file" target="_blank"
                ...
                <div class="overflow-x-auto">
                    ...
                    <div id="alert" class="flex items-center p-4 mb-4 text-yellow-800 rounded-lg bg-[#09090d]"
                        ...
                        <div class="ml-3 text-sm font-medium text-yellow-500">
                            Please <a
                                href="https://www.iorad.com/player/2327871/KeyAuth-upload-files-tutorial" target="_blank"
                                class="font-semibold underline hover:no-underline">follow this guide</a> to upload files. FREE & super easy!
                        </div>
                    </div>
                    <!-- End Alert Box -->
                    ...
                    <button 
                        id="dropdownselection" data-dropdown-toggle="filedropdown" 
                    ...
                    <div id="filedropdown" class="z-10 hidden bg-[#09090d] divide-y divide-gray-100 rounded-lg shadow w-44">
                        <ul class="py-2 text-sm text-white">
                            ...
                            <li>
                                <form method="post">
                                    <input type="hidden" name="action" value="authEnable">
                                    <input type="hidden" id="selected" name="selected">
                                    <a href="#" onclick="this.parentNode.submit();" class="block px-4 py-2 focus:bold ml-2 hover:text-red-700">Turn on authenticated</a>
                                </form>
                            </li>
                            <li>
                                <form method="post">
                                    <input type="hidden" name="action" value="authDisable">
                                    <input type="hidden" id="selected" name="selected">
                                    <a href="#" onclick="this.parentNode.submit();" class="block px-4 py-2 focus:bold ml-2 hover:text-red-700">Turn off authenticated</a>
                                </form>
                            </li>
                        </ul>
                    </div>
                            ...
                            <div class="relative bg-[#0f0f17] rounded-lg border border-blue-700 shadow">
                                <div class="px-6 py-6 lg:px-8">
                                    ...
                                    <div id="alert" class="flex items-center p-4 mb-4 text-yellow-800 rounded-lg bg-[#09090d]" role="alert">
                                        ...
                                        <span class="sr-only">Info</span>
                                        <div class="ml-3 text-sm font-medium text-yellow-500">
                                            Please <a href="https://www.iorad.com/player/2327871/KeyAuth-upload-files-tutorial" target="_blank" class="font-semibold underline hover:no-underline">follow this guide</a> to upload files. FREE &amp; super easy!
                                        </div>
                                    </div>
                                    <form class="space-y-6" method="POST">
                                        <div>
                                ...
                                if ($_SESSION['app']){
                                    ...
                                    if ($query->num_rows > 0) {
                                        while ($row = mysqli_fetch_array($query->result)){
                                            ...
                                            echo '<td> <input type="checkbox" style="outline:none;"class="w-4 h-4 text-blue-600 bg-[#0f0f17] border-gray-300 rounded focus:ring-blue-500 checkbox" value="' . $row["id"] .'">' . '</td>';
                                            ...
                                            echo "<td><span class='border " . ($row["authed"] ? 'border-green-700 text-green-700' : 'border-red-700 text-red-700') . " text-xs font-medium mr-2 px-2.5 py-0.5 rounded'>" . ($row["authed"] ? 'True' : 'False') . "</span></td>";
