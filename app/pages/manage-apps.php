...
if (isset($_POST['refreshapp'])) {
    ...
    if ($query->affected_rows != 0) {
        ...
        misc\mysql\query("UPDATE `tokens` SET `app` = ? WHERE `app` = ?", [$new_secret, $_SESSION['app']]);
...
if (isset($_POST['deleteapp'])) {
    ...
    if ($_POST['confirmappname'] !== "Confirm Deletion"){
        dashboard\primary\error("Failed!");
        echo '<meta http-equiv="refresh" content="2">';
        return;
    }
    ...
    if ($query->affected_rows != 0) {
        ...
        misc\mysql\query("DELETE FROM `tokens` WHERE `app` = ?", [$app]);
...
?>
 ...
 <?php if ($alertMsg != NULL){ ?>
        <div id="staff-alert-modal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full">
            <div class="relative w-full max-w-md max-h-full">
                <div class="relative bg-[#0f0f17] border border-blue-700 rounded-lg shadow">
                    <div class="p-6 text-center">
                        <div class="flex flex-col items-center p-4 mb-4 text-sm text-blue-800 border border-blue-700 rounded-lg bg-[#0f0f17]" role="alert">
                            <div>
                                <span class="font-medium text-blue-400"><b>New Message From Staff!</b></span>
                            </div>
                            <div class="mt-2">
                                <span class="font-medium text-blue-400"><?= $alertMsg; ?></span>
                            </div>
                        </div>
                        <form method="POST">
    ...
    <div class="mb-1 w-full bg-[#0f0f17] mt-4 md:mt-12 rounded-xl">
        <div class="mb-4 p-8">
            ...
            <p class="text-xs text-gray-500">This is where it all begins. <a
                    href="https://keyauthdocs.apidog.io/sellerapi/application-settings/retrieve-current-application-settings" target="_blank"
...
public static api KeyAuthApp = new api(
    name: "<?= $appname; ?>", // App name - VaultCord.com FREE Discord backup bot for members & your entire server saved from terms and nukes!
    ownerid: "<?= $ownerid; ?>", // Account ID
    version: "<?= $version; ?>" // Application version. Used for automatic downloads see video here https://www.youtube.com/watch?v=kW195PLCBKs
    //path: @"Your_Path_Here" // (OPTIONAL) see tutorial here https://www.youtube.com/watch?v=I9rxt821gMk&t=1s
                            ...
                            <a href="https://youtu.be/5x4YkTmFH-U?si=K9Jlm81ZvRvolGEI" target="_blank" type="button"
                                class="inline-flex text-white bg-[#0f0f17] hover:opacity-60 focus:ring-0 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 transition duration-200">
...
std::string name = skCrypt("<?= $appname; ?>").decrypt(); // App name - VaultCord.com FREE Discord backup bot for members & your entire server saved from terms and nukes!
std::string ownerid = skCrypt("<?= $ownerid; ?>").decrypt(); // Account ID
std::string secret = skCrypt("<?= $secret; ?>").decrypt(); // Application secret (not used in latest C++ example)
std::string version = skCrypt("<?= $version; ?>").decrypt(); // Application version. Used for automatic downloads see video here https://www.youtube.com/watch?v=kW195PLCBKs
...
std::string path = skCrypt("").decrypt(); // (OPTIONAL) see tutorial here https://www.youtube.com/watch?v=I9rxt821gMk&t=1s
                            ...
                            <a href="https://youtu.be/GEXpZo3sce0?si=-stUWCHlPVKFK42o" target="_blank" type="button"
                                class="inline-flex text-white bg-[#0f0f17] hover:opacity-60 focus:ring-0 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 transition duration-200">
...
$name = "<?= $appname; ?>"; // App name - VaultCord.com FREE Discord backup bot for members & your entire server saved from terms and nukes!
$ownerid = "<?= $ownerid; ?>"; // Account ID </pre> 
                            <br>
                            <button type="button"
                                class="copy-button mt-3 inline-flex text-white bg-[#0f0f17] hover:opacity-60 focus:ring-0 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 transition duration-200"
...
const KeyAuthApp = new KeyAuth(
    "<?= $appname; ?>", // App name - VaultCord.com FREE Discord backup bot for members & your entire server saved from terms and nukes!
    "<?= $ownerid; ?>", // Account ID
    "<?= $secret; ?>", // Encryption key, keep hidden and and protect this string in your code!
...
private static String ownerid = "<?= $ownerid; ?>", // Account ID
private static String appname = "<?= $appname; ?>", // App name - VaultCord.com FREE Discord backup bot for members & your entire server saved from terms and nukes!
private static String version = "<?= $version; ?>" // Application version. Used for automatic downloads see video here https://www.youtube.com/watch?v=kW195PLCBKs</pre>
                            <br>
                            <button type="button"
                                class="copy-button mt-3 inline-flex text-white bg-[#0f0f17] hover:opacity-60 focus:ring-0 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 transition duration-200"
...
Private Shared name As String = "<?= $appname; ?>" ' App name - VaultCord.com FREE Discord backup bot for members & your entire server saved from terms and nukes!
Private Shared ownerid As String = "<?= $ownerid; ?>" ' Account ID
Private Shared secret As String = "<?= $secret; ?>" ' Encryption key, keep hidden and and protect this string in your code!
Private Shared version As String = "<?= $version; ?>" ' Application version. Used for automatic downloads see video here https://www.youtube.com/watch?v=kW195PLCBKs</pre>
                            <br>
                            <button type="button"
                                class="copy-button mt-3 inline-flex text-white bg-[#0f0f17] hover:opacity-60 focus:ring-0 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 transition duration-200"
...
KeyAuthApp.Api(
    ...
    "", // Token Path (PUT NULL OR LEAVE BLANK IF YOU DO NOT WANT TO USE THE TOKEN VALIDATION SYSTEM! MUST DISABLE VIA APP SETTINGS)
                            ...
                            <button type="button"
                                ...
                                Copy Credentials</button>
                            <a href="https://github.com/KeyAuth/KeyAuth-Go-Example" target="_blank" type="button"
                                class="inline-flex text-white bg-[#0f0f17] hover:opacity-60 focus:ring-0 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 transition duration-200">
...
local name = "<?= $appname; ?>"; -- App name - VaultCord.com FREE Discord backup bot for members & your entire server saved from terms and nukes!
local ownerid = "<?= $ownerid; ?>"; -- Account ID
local version = "<?= $version; ?>"; -- Application version. Used for automatic downloads see video here https://www.youtube.com/watch?v=kW195PLCBKs</pre>
                            <br>
                            <button type="button"
                                class="copy-button mt-3 inline-flex text-white bg-[#0f0f17] hover:opacity-60 focus:ring-0 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 transition duration-200"
    ...
    "<?= $version; ?>" # Application version. Used for automatic downloads see video here https://www.youtube.com/watch?v=kW195PLCBKs
                    ...
                    <div class="mt-3 gap-1.5 grid grid-cols-1 sm:grid-cols-2 md:block md:grid-cols-0">
                        ...
                        <button
                            ...
                            Create Application <?= ($_SESSION['role'] == "tester" ? "(start by clicking here!)" : "") ?>
                        </button>
                        ...
                        <?php if ($alertMsg != NULL){ ?>
                        <button id="staff-alert-modalbutton" data-modal-target="staff-alert-modal" data-modal-toggle="staff-alert-modal" class="hidden" type="button">
  Toggle modal
</button>
<?php } ?>
                    </div>
                ...
                <div class="relative overflow-x-auto shadow-md sm:rounded-lg pt-5 pb-5 pl-3 pr-3">
                    ...
                    foreach ($rows as $row) {
                        ...
                        if (isset($_SESSION["name"])) {
                            $appSelected = ($_SESSION["name"] == $appName);
                        } else {
                            $appSelected = 0;
                        }
                    ?>
            ...
            <div class="relative w-full max-w-md max-h-full">
                ...
                <div class="relative bg-[#0f0f17] rounded-lg border border-blue-700 shadow">
                    <div class="px-6 py-6 lg:px-8">
                        <h3 class="mb-4 text-xl font-medium text-white-900">Create a new app!</h3>
                            ...
                            <div>
                                <div class="relative mb-4">
                                    <input type="text" id="confirmappname" name="confirmappname"
                                        ...
                                        placeholder="Confirm Deletion" autocomplete="on" required>
        ...
        <?php if ($alertMsg != NULL){ ?>               
            $(document).ready(function() {
                var promise = new Promise(function(resolve, reject) {
                    $('#staff-alert-modalbutton').click(function() {
                        resolve();
                    });
                });
