    ...
    if (empty(trim($_POST['mask']))) {
        ...
        switch ($key) {
            ...
            default:
                ...
                dashboard\primary\success("License(s) Created And Displayed Above!");
                break;
        }
    }
}
...
if (isset($_POST["action"]) && isset($_POST["selected"])) {
    ...
    if ($_POST["action"] == "delete") {
        ...
        foreach ($selected as $value) {
            $resp = misc\license\deleteSingular($value, false, $_SESSION['app']);
            match ($resp) {
    ...
    else if ($_POST["action"] == "banKeyMulti") {
        foreach ($selected as $value) {
            $resp = misc\license\ban($value, "You selected to ban multiple keys", 0);
            match ($resp) {
                'failure' => dashboard\primary\error("Failed to ban keys!"),
                'success' => dashboard\primary\success("Successfully banned keys!"),
                default => dashboard\primary\error("Unhandled Error! Contact us if you need help")
            };
        }
    }
        ...
        <div class="mb-4 p-4">
            ...
            <div class="p-4 flex flex-col">
                <div class="overflow-x-auto">
                    ...
                    <button id="dropdownselection" data-dropdown-toggle="licenseSelecteddropdown" 
                    ...
                    <div id="licenseSelecteddropdown" class="z-10 hidden bg-[#09090d] divide-y divide-gray-100 rounded-lg shadow w-44">
                        <ul class="py-2 text-sm text-white">
                            ...
                            <li>
                                <form method="post">
                                    ...
                                    <input type="hidden" id="selected" name="selected">
                                    <a href="#" onclick="this.parentNode.submit();" class="block px-4 py-2 focus:bold ml-2 hover:text-red-700">Ban selected</a>
                                </form>
                            </li>
                        </ul>
                    </div>
    ...
    ?>
                                        ...
                                        <div>
                                            ...
                                            <div id="alert" class="flex items-center p-4 mb-4 text-yellow-800 rounded-lg bg-[#09090d]"
                                                ...
                                                <div class="ml-3 text-sm font-medium text-yellow-500">
                                                    Free user keys will say "<a
                                                        href="https://vaultcord.com/?utm_source=keyauth" target="_blank"
                                                        class="font-semibold underline hover:no-underline">VAULTCORD</a>" because of our partnership with the Free Discord backup bot. Backup all members & your entire Discord server!
                                                </div>
                                            </div>
                                            <!-- End Alert Box -->
                                            ...
                                            <div class="flex items-center justify-between mb-4">
                                                ...
                                                <div class="flex items-center">
                                                    <input id="capitalLetters" name="capitalLetters" type="checkbox"
                                                        class="w-4 h-4 text-blue-600 bg-[#0f0f17] border-gray-300 rounded focus:ring-blue-500 focus:ring-2"
                ...
                if (isset($_POST['importkeysFile'])) {
                    if ($_FILES['file_input']['error'] == UPLOAD_ERR_OK) {
                        ...
                        if (!isset($jsonArray['keys'])) {
                            dashboard\primary\error("Missing 'keys' array in JSON!");
                            echo "<meta http-equiv='Refresh' Content='1;'>";
                            return;
                        }

                        if (!isset($jsonArray['tokens'])) {
                            dashboard\primary\error("Missing 'tokens' array in JSON!");
                            echo "<meta http-equiv='Refresh' Content='1;'>";
                            return;
                        }
                
                        foreach ($jsonArray['keys'] as $keyData) {
                            $key_format = $keyData['key'];
                            $note_format = $keyData['note'];
                            $expiry_format = $keyData['expiry'];
                            $status_format = $keyData['status'];
                            $level_format = $keyData['level'];
                            $genby_format = $keyData['genby'];
                            $gendate_format = $keyData['gendate'];
                            $usedon_format = $keyData['usedon'];
                            $banned_format = $keyData['banned'];
                            ...
                            misc\mysql\query("INSERT INTO `keys` (`key`, `note`, `expires`, `status`, `level`, `genby`, `gendate`, `usedon`, `app`, `banned`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [$key_format, $note_format, $expiry_format, $status_format, $level_format, $genby_format, $gendate_format, $usedon_format, $_SESSION['app'], $bannned_format]);;
                        }

                        foreach ($jsonArray['tokens'] as $tokenData){
