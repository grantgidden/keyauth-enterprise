...
if (isset($_POST["selected"]) && isset($_POST["action"])) {
    ...
    if ($_POST["action"] == "delete") {
        foreach ($selected as $webhook) {
            $resp = misc\webhook\deleteSingular($webhook);
            match($resp){
                'failure' => dashboard\primary\error("Failed to delete webhook!"),
                'success' => dashboard\primary\success("Successfully deleted webhook!"),
                default => dashboard\primary\error("Unhandled Error! Contact us if you need help")
        ...
        foreach ($selected as $value) {
            $query = misc\mysql\query("UPDATE `webhooks` SET `authed` = 1 WHERE `app` = ? AND `webid` = ?", [$_SESSION['app'], $value]);

            if ($query->affected_rows != 0) {
                misc\cache\purge('KeyAuthFile:' . ($secret ?? $_SESSION['app']) . ':' . $fileid);
                dashboard\primary\success("Successfully updated webhooks!");
            } else {
                dashboard\primary\error("Failed to update webhooks");
            }
        }
    }
    else if ($_POST["action"] == "authDisable") {
        foreach ($selected as $value) {
            $query = misc\mysql\query("UPDATE `webhooks` SET `authed` = 0 WHERE `app` = ? AND `webid` = ?", [$_SESSION['app'], $value]);
    ...
    <div class="mb-1 w-full bg-[#0f0f17] rounded-xl">
        <div class="mb-4 p-4">
            ...
            <p class="text-xs text-gray-500">Send and receive secure requests. <a
                    href="https://keyauthdocs.apidog.io/api/features/webhook" target="_blank"
            ...
            <div class="p-4 flex flex-col">
                <div class="overflow-x-auto">
                    ...
                    <button id="dropdownselection" data-dropdown-toggle="webhookdropdown" 
                    ...
                    <div id="webhookdropdown" class="z-10 hidden bg-[#09090d] divide-y divide-gray-100 rounded-lg shadow w-44">
            ...
            if ($_SESSION['app']) {
                ...
                if ($query->num_rows > 0) {
                    while ($row = mysqli_fetch_array($query->result)) {
                        ...
                        echo '<td> <input type="checkbox" style="outline:none;"class="w-4 h-4 text-blue-600 bg-[#0f0f17] border-gray-300 rounded focus:ring-blue-500 checkbox" value="' . $row["webid"] .'">' . '</td>';
