...
function deleteAllChannels($secret = null){
        $query = mysql\query("DELETE FROM `chats` WHERE `app` = ?",[$secret ?? $_SESSION['app']]);

        if ($query->affected_rows > 0) {
                $query = mysql\query("DELETE FROM `chatmsgs` WHERE `app` = ?",[$secret ?? $_SESSION['app']]);
                cache\purge('KeyAuthChatMsgs:' . ($secret ?? $_SESSION['app']));
                if ($_SESSION['role'] == "seller" || !is_null($secret)){
                        cache\purge('KeyAuthChats:' . ($secret ?? $_SESSION['app']));
                } 
                return 'success';
