...
function add($username, $sub, $expiry, $secret = null, $password = null)
{
        ...
        $subQuery = mysql\query("SELECT * FROM `subs` WHERE `subscription` = ? AND `paused` = 1 AND `app` = ?", [$sub, $secret ?? $_SESSION['app']]);
...
function changeUsername($oldUsername, $newUsername, $secret = null)
{
        ...
        if ($query->affected_rows > 0) {
                ...
                mysql\query("UPDATE `tokens` SET `assigned` = ? WHERE `assigned` = ? AND `app` = ?", [$newUsername, $oldUsername, $secret ?? $_SESSION['app']]);
...
function changePassword($username, $password, $secret = null)
{
        ...
        if ($query->affected_rows > 0) {
                ...
                if ($_SESSION['role'] == "seller" || !is_null($secret)) {
                        cache\purge('KeyAuthUsers:' . ($secret ?? $_SESSION['app']));
                        cache\purge('KeyAuthUserData:' . $_SESSION['app'] . ':' . $username);
                }
                return 'success';
        } else {
                return 'failure';
...
function changeEmail($username, $email, $secret = null){
        ...
        $query = mysql\query("UPDATE `users` SET `email` = SHA1(LOWER(?)) WHERE `username` = ? AND `app` = ?", [$email, $username, $secret ?? $_SESSION['app']]);
        if ($query->affected_rows > 0){
                cache\purge('KeyAuthUser:' . ($secret ?? $_SESSION['app']) . ':' . $username);
                if ($_SESSION['role'] == "seller" || !is_null($secret)){
                        cache\purge('KeyAuthUsers:' . ($secret ?? $_SESSION['app']));
