...
switch (hex2bin($_POST['type'])) {
    case 'init':
        ...
        if($newSession) {
            ...
            misc\mysql\query("REPLACE INTO `sessions` (`id`, `app`, `expiry`, `created_at`, `enckey`,`ip`) VALUES (?, ?, ?, ?, ?, ?)", [$sessionid, $secret, $time, time(), $enckey, $ip]);
    ...
    case 'ban':
        ...
        $query = misc\mysql\query("UPDATE `users` SET `banned` = ? WHERE `username` = ? AND `app` = ?", [$reason, $credential, $secret]);
