...
if($_POST['ownerid'] != "OEYrdeqNme"){
    $rateLimit = ($_POST['ownerid'] != "OEYrdeqNme") ? 850 : 350;

    if (misc\cache\rateLimit("KeyAuthAppLimit:" . ($_POST['ownerid'] ?? $_GET['ownerid']), 1, 60, $rateLimit)){
        die(json_encode(array("success" => false, "message" => "This application has sent too many requests. Try again later.")));
    }
}
...
$enabledFunctions = (int)$row['enabledFunctions'];
...
switch ($_POST['type'] ?? $_GET['type']) {
    case 'init':
        ...
        if($newSession) {
            ...
            misc\mysql\query("REPLACE INTO `sessions` (`id`, `app`, `expiry`, `created_at`, `enckey`, `ip`) VALUES (?, ?, ?, ?, NULLIF(?, ''), ?)", [$sessionid, $secret, $time, time(), $enckey, $ip]);
            misc\cache\insert("KeyAuthSessionDupe:$secret:$ip", $sessionid, $sessionexpiry);
        }
		
		if($duplicateSession) {
			misc\mysql\query("update `sessions` set `enckey` = ? where `id` = ?", [$enckey, $sessionid]);
		}
    ...
    case 'register':
        if (!($enabledFunctions & 2)) {
            ...
            die(json_encode(array(
                "success" => false,
                "message" => "This function has been disabled by app owner, it can be enabled in Function Management on https://keyauth.cc/app/?page=app-settings."
            )));
        } 
    ...
    case 'file':
        ...
        if ($row == "not_found") {
            ...
            $sig = !is_null($enckey) ? hash_hmac('sha256', $response, $enckey)  : 'No encryption key supplied';
            header("signature: {$sig}");
            http_response_code(201); // putting a number above 200 so cloudflare worker won't cache
...

        {
            if (!$session["validated"]) {
                ...
                $sig = !is_null($enckey) ? hash_hmac('sha256', $response, $enckey)  : 'No encryption key supplied';
                header("signature: {$sig}");
                http_response_code(202); // putting a number above 200 so cloudflare worker won't cache
                die($response);
            }
        }
        ...
        if ($statusCode == 403 || $statusCode == 404) {
            ...
            $sig = !is_null($enckey) ? hash_hmac('sha256', $response, $enckey)  : 'No encryption key supplied';
            header("signature: {$sig}");
            http_response_code(203); // putting a number above 200 so cloudflare worker won't cache
            die($response);
        }
