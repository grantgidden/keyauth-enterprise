...
<body>
            ...
            <div class="w-full md:w-1/2 md:p-8 -ml-4 md:-ml-0">
                <div class="p-2 md:p-4 py-16 flex flex-col justify-center h-full">
                    <form class="md:max-w-md md:ml-32 space-y-4 md:space-y-6" method="post">
                        ...
                        <div class="relative mb-4">
                            <input type="password" id="password" name="password" autocomplete="new-password" class="block px-2.5 pb-2.5 pt-4 w-full text-sm text-white bg-transparent rounded-lg border-1 border-border-gray-300 appearance-none focus:ring-0 peer" placeholder=" " data-popover-target="popover-password" data-popover-placement="bottom" autocomplete="on" required>
                        ...
                        <div>
                            <p class="mt-4 px-8 text-center text-sm text-muted-foreground">
                                By creating an account, you agree to our <a href="/terms" class="underline underline-offset-4 hover:text-primary">
                                Terms of Service</a> and <a href="/privacy" class="underline underline-offset-4 hover:text-primary">
                                Privacy Policy</a>
                        </div>
    ...
    if (isset($_POST['register'])) {
        ...
        if (misc\etc\isPhonyEmail($email)) {
            ...
            dashboard\primary\wh_log($logwebhook, "{$username} has failed email validation with `{$email}`", $webhookun, "db1010", "No ping");
            return;
        }
        if (misc\etc\isBreached($password)) {
            dashboard\primary\wh_log($logwebhook, "{$username} attempted to register with leaked password `{$password}`", $webhookun, "db1010", "No ping");
        ...
        dashboard\primary\wh_log($logwebhook, "{$username} has registered successfully", $webhookun, "41db10", "No ping");
        $htmlContent = "<html>
                                        <body>
                                                <h1>Welcome!</h1>
                                                ...
                                                <p>KeyAuth API documentation can be seen here <a href=\"https://keyauthdocs.apidog.io\">https://keyauthdocs.apidog.io</a></p>
